...
setlocal
set "SCRIPT_DIR=%~dp0"
set "EXE=%SCRIPT_DIR%dist\AdientPVSService.exe"
set "SVC=AdientPVS"
...
if not exist "%EXE%" (
  ...
  echo Run build_pvs_exe.ps1 first to create the executable.
  exit /b 1
)
...
"%NSSM%" set "%SVC%" AppStdout "%LOGDIR%\pvs_stdout.log"
"%NSSM%" set "%SVC%" AppStderr "%LOGDIR%\pvs_stderr.log"
